...
const unsigned long MAX_RECORDING_LENGTH_MS = 30000;  // 30 seconds

// This is the IP Address and port that the server.js node server is running on.
IPAddress serverAddr = IPAddress(192, 168, 1, 1); // your node servers ip goes here
int serverPort = 7123;
...
void setup() {
    ...
    waitFor(Serial.isConnected, 10000);  // Wait up to 10 seconds for serial to connect

    // output size is typically Microphone_PDM::OutputSize::UNSIGNED_8 or microphone_PDM::OutputSize::SIGNED_16
    ...
    // The sample rate default to 16000. The other valid value is 8000. .withSampleRate(16000)
    int err = Microphone_PDM::instance()
                  .withOutputSize(Microphone_PDM::OutputSize::SIGNED_16)  // or Microphone_PDM::OutputSize::UNSIGNED_8
                  .withRange(Microphone_PDM::Range::RANGE_2048)
                  .withSampleRate(32000)  // .withSampleRate(16000) is default and works! | .withSampleRate(8000)
                  .init();
...
void loop() {
    switch (state) {
        case STATE_WAITING:
            ...
            break;  // nothing to do, else we'll fall through to STATE_CONNECT
